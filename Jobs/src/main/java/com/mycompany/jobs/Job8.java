...
public class Job8 {
      ...
      if(fields.length>=25){
        ...
        for(int i=5;i<26;i++){
          if("".equals(fields[i])){ 
              ...
              String decade = "2020s";
              if(i<=14) decade = "2000s";
              if(i>14&&i<=24) decade = "2010s";
              subregion.set(subRegionName+","+decade+","+sex);
  ...
  public static class CustomTextOutputFormat extends TextOutputFormat<Text, Text> {
    private static final String HEADER = "subRegion,decade,sex,average,maximum,minimum";
